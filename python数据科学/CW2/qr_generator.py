...
class QRCodeApp(QMainWindow):
    ...
    def generate_qr(self):
        ...
        try:
            ...
            # 获取完整编码数据和 ECC 位
            segs = QrSegment.make_segments(input_text)
            bb = _BitBuffer()
            for seg in segs:
                bb.append_bits(seg.get_mode().get_mode_bits(), 4)
                bb.append_bits(seg.get_num_chars(), seg.get_mode().num_char_count_bits(version))
                bb.extend(seg._bitdata)
            datacapacitybits = QrCode._get_num_data_codewords(version, QrCode.Ecc.LOW) * 8
            bb.append_bits(0, min(4, datacapacitybits - len(bb)))
            bb.append_bits(0, -len(bb) % 8)
            for padbyte in itertools.cycle((0xEC, 0x11)):
                if len(bb) >= datacapacitybits:
                    break
                bb.append_bits(padbyte, 8)
            encoded_data = bb  # 完整编码数据位
            rs = RSCodec(QrCode._ECC_CODEWORDS_PER_BLOCK[QrCode.Ecc.LOW.ordinal][version])
            data_bytes = input_text.encode('utf-8')
            ecc_bytes = rs.encode(data_bytes)[len(data_bytes):]  # 获取 ECC 字节
            ecc_bits = [bit for byte in ecc_bytes for bit in [(byte >> i) & 1 for i in range(7, -1, -1)]]  # 转换为位列表
            ...
            # 保存中间数据
            self.save_intermediate_data(encoded_data, ecc_bits)
    ...
    def save_intermediate_data(self, encoded_data: list[int], ecc_bits: list[int]):
        """保存编码数据和 ECC 位到文件"""
        with open("intermediate_data.txt", "w", encoding="utf-8") as f:
            f.write("Encoded Data (Bits):\n")
            f.write(" ".join(str(bit) for bit in encoded_data) + "\n\n")
            f.write("ECC Bits:\n")
            f.write(" ".join(str(bit) for bit in ecc_bits) + "\n")
